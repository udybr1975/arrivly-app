...
import { asPromptScalar } from './_lib/prompt-scalar.js'
...
async function buildPublicSystemInstruction(
  ...
  // _lib/prompt-scalar.ts. Same LATENT framing as the guest surface: every value here is
  ...
  return [
    `You are the friendly concierge for "${asPromptScalar(brandName, 60)}", helping a guest who has booked "${asPromptScalar(apt.name, 80)}"${where ? ` in ${where}` : ''} and is planning their trip BEFORE they arrive.`,
    ``,
