...
import { asPromptScalar } from './prompt-scalar.js'
...
// Short scalars are fenced via _lib/prompt-scalar.ts; the full reasoning for every step of
// that fence lives there, not here (PG-38).
...
export async function buildGuestSystemInstruction(
  ...
  // Fenced ONCE into shared consts and reused by both joins below (PG-33): the same three
  // columns used to be fenced in two separate expressions, which a future edit could have
  ...
  const fullAddress = [streetLine, nb, city, country].filter(Boolean).join(', ')
