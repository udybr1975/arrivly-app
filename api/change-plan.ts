...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { data: host } = await admin
      .from('hosts')
      .select('stripe_subscription_id, subscription_status, tier, pending_tier, name, contact_email, is_test')
      .eq('id', userId)
      .maybeSingle()
    ...
    if (sub.status === 'trialing') {
      ...
      if (newTier === currentTier) {
        ...
        await Promise.allSettled([
          ...(contactEmail && !isTestHost ? [sendEmail({
            to: contactEmail,
          ...
          sendNtfy({
            title: 'Bemgu',
            message: `A host undid a scheduled change — staying on ${TIER_NAMES_C[currentTier] ?? `Tier ${currentTier}`}`,
            priority: 'default',
          }),
        ])
        ...
        await Promise.allSettled([
          ...(contactEmailD && !isTestHost ? [sendEmail({
            to: contactEmailD,
          ...
          sendNtfy({
            title: 'Bemgu',
            message: `A host scheduled a downgrade to ${TIER_NAMES_C[newTier] ?? `Tier ${newTier}`} (${formatMoney(newPlanPriceCents, newPlanCurrency)}/mo) — effective ${effectiveAt.split('T')[0]}`,
            priority: 'default',
          }),
        ])
