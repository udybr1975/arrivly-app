...
2. ~~**BUG — "Manage subscription" not shown for trialing subscribers.**~~ **FIXED** (`0ae77a5` + full BillingPanel rewrite). `BillingPanel` now gates on `stripe_subscription_id` presence and exposes Upgrade/Downgrade/Cancel controls in `manageMode`. Confirm Stripe billing portal has cancellation enabled.
