...
3. **Dependency triage — the current figure is GitHub's 11 (6 high, 5 moderate), measured on the 28 Aug push after the react-router bump (`386faf0`) cleared five**; the remainder is server-side or build/dev-only — see PG-17 in docs/pentest-queue.md for the triage. Routine npm-update pass before the flip. **Older counts in this file (Known notes' 8, and a since-deleted 7 on `d254df9`) are SUPERSEDED, and the gap between them was never drift** — GitHub counts one alert per advisory per manifest path while `npm audit` dedupes per package, so the two tools measure different things. Precedes the pentest gate.
