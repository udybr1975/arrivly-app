...
> **Current HEAD:** 71a974c (2026-06-09)
...
- **`api/change-plan.ts` cancel-pending-before-cancel guard.** If a subscription schedule is attached, `cancel_at_period_end` on the sub is blocked (409 `pending_change_in_progress`). Hosts must undo the pending tier change before they can cancel. This is intentional — a schedule and a `cancel_at_period_end` flag fighting over the same period end would produce undefined Stripe behaviour.
...
- **E — Billing (Tier-1 Stripe):** E1 DONE ✓ (`23a4abd`) — create-subscription + billing-portal + `api/_lib/stripe.ts`, verified in Stripe TEST sandbox. E2 DONE ✓ (`bb077e6`) — stripe-webhook (raw-body, signature verify, Arrivly isolation, idempotent sync); lifecycle emails; guest-page host fetch via SECURITY DEFINER RPC; `showPoweredBy` = trial || grace. E2.1 DONE ✓ (`a18f9ec`) — 7-channel subscription-change fan-out; dismissible `billing_notice` banner; ntfy live. E3 DONE ✓ (`b827ecb`→`71a974c`) — in-app plan switching (`change-plan.ts`), cancellation/resume (`cancel-subscription.ts`), BillingPanel full manage-mode rewrite, deferral bug fix, webhook notice + triple-fire fix. **Remaining before billing goes live:** complete 360 webhook test (Scenarios 1+4 need re-test with fixed classification); request-time confirmation emails (DEFERRED — iterate with Udy first); re-target UX (DEFERRED); set live Stripe keys/prices/webhook.
...
2. **Complete 360 webhook test** — Scenarios 2 (upgrade T1→T2) and 3 (downgrade T2→T1) PASSED. Scenarios 1 (new subscription → exactly ONE 'started' notice, not 'downgraded', not 3×) and 4 (cancel → exactly ONE 'cancelled' notice, not 'upgraded') are UNBLOCKED by the S14 classification fix — re-test via `admin_audit` (one event per scenario). In-app cancel now works (`cancel-subscription.ts`), so Scenario 4 no longer requires the Stripe dashboard.
3. **E2.1 advanced test-clock scenarios** — 5 (payment failure → grace) and 6 (renewal → no fan-out/silence).
4. **DEFERRED — Request-time confirmation emails** (do NOT build until Udy iterates the subscription UX): fire a host email on scheduled change / undo / cancel / resume with "changes on [date]" copy (not "effective immediately"); keep the apply-time webhook email at the boundary; do NOT double-email on the trial-immediate path. Builders needed in `api/_lib/email.ts`; wire into `change-plan.ts` (scheduled/reverted only) and `cancel-subscription.ts` (cancel/resume). Existing builders for reference: `subscriptionStartedEmail`, `subscriptionChangedEmail` ("effective immediately"), `subscriptionCancelledEmail`, `subscriptionPastDueEmail`, `adminSubscriptionEventEmail`; `layout(heading, bodyHtml, ctaLabel, ctaUrl)`; FROM `hello@anna-stays.fi`.
5. **DEFERRED — Re-target UX** (do NOT build until Udy iterates): don't lock other plan buttons when a change is scheduled — clicking another non-current tier should re-target the existing schedule (backend already supports this); make the current-plan card the "cancel scheduled change"; clearer banner copy. Removes the current "undo then switch" two-step.
6. **Verify `last_billing_notice_sig` column grant** — confirm UPDATE is revoked from `authenticated` role in the Supabase dashboard (consistent with all other server-only columns; flagged by security-auditor S14).
7. **Add-property flow + property-cap enforcement** — still unbuilt; fold into existing dashboard vs. separate page TBD.
8. **Live/Draft publish toggle** on the dashboard property card — no UI control exists today; properties created before the `is_visible=true` fix are stuck in Draft and can only be published via DB.
9. **Guide hardening:** treat a 0-place result as a soft failure — don't overwrite a good guide with an empty one (`placeCount > 0` guard before upsert); surface a retry to the host.
10. **Signup → card-at-onboarding (product decision pending):** collect card + choose tier during signup so trials convert automatically. Open decisions: hard vs soft card requirement; trial vs immediate charge; which tiers at signup. Also: drive trial clock from the Stripe subscription (not a second app clock), and add EU/Finland auto-renewal disclosure. Build AFTER 360 test passes.
11. **Live Stripe keys** — when ready to go live: create live products/prices/webhook in Stripe, set `STRIPE_SECRET_KEY`/`STRIPE_PRICE_TIER_*`/`STRIPE_WEBHOOK_SECRET` to live values in Vercel, **re-verify price→tier mapping for LIVE prices** (the env-var fix in S14 corrected test prices; verify live prices match the same pattern). Redeploy.
12. **Cosmetic:** Dashboard "City guide" completeness indicator is hardcoded to "–" (`check(false)`).
