...
> **Current HEAD:** f1ef316 (Session 20, Jun 23 2026 — Phase G hardening underway: iCal SSRF fix + city-events converted to DB cache). Prior: 360a987 (S19 cont.).
...
**Immediate next (Phase G, in progress — S20):** rate-limit `api/guest-chat.ts` — the one live, interactive, anonymous AI endpoint that can't be cached (a chatbot can't be pre-generated), so a per-instance rate limit (keyed apartmentId+IP, mirroring the existing inline limiter pattern) is the correct defence. NOTE: `daily-greeting` is already protected (only verified-booking guests trigger Gemini, and it's per-day cached) and `generate-guide` is already host-auth+ownership-gated — neither needs a limiter; do NOT touch them. Then remaining Phase G: server-side upload size cap (`create-upload-url.ts`), message-retention ~90-day cleanup, cron batching at scale, mobile-drawer a11y, dead-code sweep, full security-auditor pass, and finally the **pentest/"hacker" agent gate** (hard, before any live flip).
