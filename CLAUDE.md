...
> **Current HEAD:** b3b8c23 (2026-06-09)
...
4. **DONE (S15) — Request-time emails + immediate upgrades + unlocked panel.** Host+admin emails fire on scheduled change / undo / cancel / resume; active-sub upgrades are immediate with a prorated charge; the billing panel no longer locks during a pending downgrade. See the S15 Shipped entry.
5. **NEXT BUILD — 2-day-before change reminder cron.** Add daily cron `api/cron-change-reminder.ts` (guard with `isCronAuthorized`/`CRON_SECRET`; register in `vercel.json crons[]`). New DB column `change_reminder_sent_at timestamptz` (apply via Supabase MCP `apply_migration` BEFORE the cron prompt). ~2 days before a pending downgrade or scheduled cancellation effective date, send host + admin reminder emails; atomic claim on `change_reminder_sent_at` to prevent double-send; clear the flag on undo/resume or once the change applies. `formatMoney` + builders already in `api/_lib/email.ts`.
