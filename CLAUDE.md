...
>
> **LIFT — 9 Sep 2026, LIFTED BY UDY IN CHAT, anchor fix under the announcement-bar scope;
> re-frozen at `ANCHOR_SHA`.** `/#founding-hosts` landed at the top of the page.
>
> **THE RULE THIS CREATED — A HASH ANCHOR ON AN SPA NEEDS AN EXPLICIT SCROLL, AND ONE OWNER FOR
> THE ID.** Two separate defects hid behind one report: the published URL used an id the page
> never had (the code was internally consistent, so in-page clicks worked and only the URL
> failed), AND direct hash navigation never worked at all — on a fresh load the browser
> hash-scrolls BEFORE React renders the section, finds nothing and gives up silently, with
> nothing to retry it. **A section id, its links and its scroll target must all derive from ONE
> constant**, and **the initial-load scroll must be performed explicitly after mount.**
> **A PROMISE-GATED UI STATE NEEDS A TIMER, NOT JUST `.then`/`.catch`:** the scroll waits for
> the founding-status fetch so the bar has taken its space, and a fetch that never SETTLES is
> neither success nor rejection — without the 1200ms fallback the deep link would silently
> reproduce the very bug being fixed. `scroll-mt-20` clears the sticky header, or the scroll
> succeeds and still reads as broken. **Residual:** a legacy `/#founding` visitor keeps the
> stale hash in the address bar (accepted on initial load only; nothing renders or links to it).
>
> **LIFT — 9 Sep 2026, LIFTED BY UDY IN CHAT, for the Founding Hosts ANNOUNCEMENT BAR; re-frozen
> at `2d6ae8e` the same day.** A slim gold bar at the very top of `Landing.tsx`, above the sticky
...
> as an explicit extension after the security gate found that anchor is the only same-origin
> full-navigation exit off a guest page). **`Landing.tsx` was NOT touched** — the parked LANDING
> BATCH still owns its own lift.
>
> **THE RULES THIS CREATED, and they bind every future edit — full reasoning in the header
> comments of `src/lib/analytics.ts` and `src/App.tsx`:**
> - **NEVER add a react-router `<Link to>` / `<Navigate to>` into `/guest` or `/w/`.** The route
>   gate is an **ENTRY** gate: once gtag.js is in the document it lives for that document's life
>   and its enhanced-measurement hits read `window.location.href` themselves. Every guest link in
>   the app is a FULL DOCUMENT LOAD, and that is load-bearing, not incidental.
> - **A privacy control must be a MECHANISM, never a falsy value handed to someone else's
>   script.** `page_referrer: ''` looked like suppression and was a bet on how a minified
>   third-party script treats an empty string; the origin root is the fix.
> - **react-router matches routes CASE-INSENSITIVELY** (`caseSensitive = false`), so any
>   path-prefix security gate must lowercase first, or `/Guest?apt=…&token=…` walks through it.
> - **A URL-shaped identifier can ride in the PATH, not only in `?` and `#`** — the apartment
>   UUID does, at `/dashboard/property/:aptId`. Normalise over the VALUE SHAPE, not the one route.
> - **`/legal/guest-notice` is a GUEST SURFACE even though it is not a guest route** — it is
>   reached by `<Link>` from every guest screen, and Bemgu is a PROCESSOR for guest data.
> - **`?checkout=success` ON `/dashboard` IS LOAD-BEARING** — `PrivateRoute` reads it to admit a
>   host before the webhook writes `stripe_subscription_id`. Do not strip it; latch instead.
>
> **NEW ENV VAR: `GA4_API_SECRET`** — server-side only, NO `VITE_` prefix, Sensitive. Unset =
> the server conversion event silently skips, by design.
>
> **OPEN, NOT BLOCKING — GAP 11 in docs/legal-data-inventory-2026-07-28.md, for the lawyer
> fast-follow:** no in-product consent-withdrawal toggle (Art. 7(3) — needs its own lift into
> `Settings.tsx`); the GA4 data-retention setting in the Google property is unset; the US
> transfer mechanism for Google Analytics; and the legal basis for the server-side event.
>
> **LIFT — 8 Sep 2026, LIFTED BY UDY IN CHAT, for the legal-publication v1 build; re-frozen
> at `0d2e7d6` the same day.** Shipped: the public `/legal` pages (the four documents rendered
> FROM `docs/*.md` as the single source of truth), real consent links on Signup, the guest
> privacy notice linked from EVERY guest and welcome surface in EVERY state, and **D4 — the
> server-side weather proxy**, which is what makes the published notice's "nothing about you
> is sent to the weather service" sentence true. Three gate rounds, both gates each round;
> R3 BOTH PASS / 0 must-fix. **`api/weather.ts` IS A NEW PUBLIC ENDPOINT THE CLOSED HACKER
> PASS (`47bb840`) NEVER SAW.** Residuals PG-50..PG-53. Full record: docs/pentest-queue.md
...
>   hit this"; broken, now, for someone real.
> - **(b) a fix generated BY the hacker-agent pass itself.**
>
> **EVERY exception is logged in `docs/pentest-queue.md` under FREEZE EXCEPTIONS LOG**, with its
> date, SHA, category and reason — never taken as routine work. Feature work, refactors and
> cosmetic polish WAIT. An empty log is the evidence the freeze held.
>
> **THE FREEZE IS A DISCIPLINE, NOT A LOCK.** To make any other change, the freeze must be
> **CONSCIOUSLY LIFTED IN CHAT BY UDY**, the change made, and the surface **re-frozen at a new
> SHA recorded here**. Lifting is deliberate and visible. **Nothing changes by accident** — and a
> change that quietly skips the lift is not a small liberty, it is the freeze not existing.
>
...
>
> **THE FOUR THINGS BLOCKING LAUNCH:** (1) ~~Gemini billing~~ — dissolved by the ZERO-GOOGLE AI PILOT; Google is leaving the stack, there is no billing flip. (2) the legal/compliance workstream — inventory DONE, **eight gaps open**, documents 3/4/5 DRAFTED but unpublished, (the retention crons that gated publication SHIPPED 11 Aug 2026). (3) ~~gemini-2.5-flash 16 Oct shutdown~~ — CLOSED 26 Aug 2026 as OPTION A (`6518da7`): the model id is now `GEMINI_MODEL_CHAT` (unset by design, default `gemini-2.5-flash`). Google's deprecations page (13 Aug 2026) lists the model as 'no shutdown date announced'; NO REPOINT because free-tier Search grounding exists ONLY on the 2.5 line (Google pricing page, same date) — a 3.x value here means dropping grounding, not swapping a string. (4) the pentest gate. Also open but smaller: welcome-page Part 2 and the pre-live additions.
>
...
## Design System
See the Design system — GROUND TRUTH block at the top of this file. The pre-S26 list that lived here is in docs/history.md (26 Aug evening, S6).
...
- `cron-refresh-events` runs `mapPool(units, 1, …)` — concurrency 1 is the ONLY width that fits
  the 8,000 TPM Groq ceiling (verified at source 22 Aug 2026). `cron-sync-ical` batch-at-scale
...
- **RESIDUALS FROM `60a4c2b` (the four UI items) — three items, one line each; full argument in
...
- **A CAREFUL SELF-ASSESSMENT IS NOT A PROVIDER RULING (proved by Viator, 4 Aug 2026).** The
  terms were read carefully, the risk was spotted, the question was asked anyway — and the answer
  came back NO. **Ask the provider IN WRITING before selling on your own terms reading.**
- **THE EXPIRY CLASS IS A STANDING SWEEP, NOT A ONE-OFF (26 Aug 2026).** Any claim in this file
  asserting a GAP, an UNVERIFIED state or a PENDING answer is a claim WITH AN EXPIRY and nothing
  re-checks it — **re-verify against source before keeping it.** A lesson that is merely old is
  not the problem; **a lesson that is WRONG is.**
...
- **FIRST-VISIT GUEST FLOWS: FRESH PROFILE, LOCAL STATE FIRST, ABSENCES COUNT (Aug 22 2026,
  `118d05f`).** Test in a profile with no service worker and no stored token (two independent
  reasons a broken path looks fine); when two devices disagree, suspect SW registration /
  localStorage before the platform; read a network trace for requests with NO status, not only
  completed ones; verify against the source surface, never a transcription; a no-oracle endpoint
  (`welcome-claim`) is opaque by design — budget the diagnostic cost, do not weaken it.
...
## PHASE I — EXPERIENCE CONNECTORS (Stages 0/4A/4B shipped; Stage 5 reporting ingest remains)
...
- **`api/backfill-canonical-city` — BLOCKED** on the CRON_SECRET-is-Sensitive decision (see
  Lessons); idempotent, no schedule. Re-count the visible fleet before running (3 apartments, all
  Helsinki, 23 Aug 2026 — unpublishing shrank the visible work, it did not do it); watch
  `resolvedNoKey`.
- **PRE-LIVE — WRITTEN CONFIRMATION FROM GYG AND TIQETS — CLOSED as LAUNCH BLOCKER 5, 1 Sep 2026** (both confirmed in writing); full history in docs/history.md, 2 Sep 2026.
- **THE QUEUE (updated 24 Aug 2026). In order:**
  1. `118d05f` residuals: a worker UPDATE mid-claim aborts the POST (seconds, post-deploy only);
     a hard-reloaded tab spends its latch on the next deploy. Bounded, never stale-serving.
  2. `welcome_show_address` stays NOT NULL DEFAULT true, no host UI — decided 24 Aug 2026
     (platforms show the address to the same person). Do not reopen; do not claim the address is
     hidden.
  6. **THE ORDER — REPLACED 26 Aug 2026. THIS IS THE AUTHORITATIVE QUEUE.**
     0-3 DONE, moved -> docs/history.md 2 Sep 2026.
     > Full DONE-entry records: docs/history.md, 2 Sep 2026.
...
**STILL PARKED, unchanged:** block-source message fix (before Founding Hosts) · pre-arrival
  ...
  shipped 26 Aug; grounding replacement is the only reason to evaluate this · **the `api/` typecheck gap** (`api/` is outside
...
- **NO CRON HEARTBEAT — "never ran" is undetectable as a CLASS.** The failure ntfy covers "ran
  and failed" only. The guide cron HAS run (15 Aug 2026) and idles correctly inside its 25-day
  gate; 3/run is a LOWER BOUND, so the ~50-apartment capacity figure stays an ESTIMATE.
  `city-events-host` (9) and `sync-ical` (15) will false-positive "block this host" on a
  legitimate 12-property setup sweep — revisit when a real portfolio host exists.
...
> Build order G → H → I → F; Stripe LIVE flip last; Phase F ships AFTER the flip (Udy,
> 28 Jul 2026) and needs its own second security pass before Tier 4 is sold.
...
## AI MODELS — provider assignment and keys
`gemini-2.5-flash` has NO announced shutdown date (Google deprecations page, 13 Aug 2026); the earlier 16 Oct 2026 date was withdrawn. TWO surfaces run live on Google: `api/guest-chat.ts` (model from `GEMINI_MODEL_CHAT`, default `gemini-2.5-flash`, grounded, `GEMINI_API_KEY_CHAT`) and `api/welcome-chat.ts` (`gemini-3.1-flash-lite`, ungrounded, `GEMINI_API_KEY_PUBLIC`). Both free tier, no card. Grounding is free ONLY on the 2.5 line; every 3.x model needs the paid tier for it — the reason guest-chat stays on 2.5.

**SHIPPED 26 Aug 2026 as `6518da7` (env binding only, no repoint — see above).** **OPTION B (`groq/compound`, which ships built-in web search and would take Google to zero) stays PARKED WITHOUT A DEADLINE — option A buys the time to evaluate it properly.** Pilot Step 6 (the chat router) was never built, and with the deadline withdrawn there is no date for it to clear. Quota measurements, the per-project model and the migration analysis are in docs/pilot-history.md.
...
| `GEMINI_API_KEY_CHAT` | gen-lang-client-0221179352 | `guest-chat` — live on Google (with `welcome-chat`) |
| `GEMINI_API_KEY_EVENTS` | gen-lang-client-0131909896 | `_lib/city-events` |
| `GEMINI_API_KEY_PUBLIC` | (separate, no card) | `welcome-chat` — live on Google, `gemini-3.1-flash-lite` |
