...
- New `api/_lib/ntfy.ts` — POST to `NTFY_URL`; `https://`-only scheme guard; ASCII-only `Title`/`Priority` headers (non-ASCII causes ByteString errors on Vercel); 500-char body cap; 5s timeout; never throws. `NTFY_URL` = `https://ntfy.sh/arrivly-subscription-plan-99` (set in Vercel Production). **[SUPERSEDED — this topic was retired post-rebrand; the current NTFY_URL topic is a different name that has never been written into this repo. Recorded so a future audit does not re-flag this archived value as a live leak.]**
