...
| B5. Guest ↔ host messaging | Message text (guest- and host-written, up to 4000 chars — free text, so guests may type anything including sensitive information), sender role, timestamps, read state, linked booking. | Direct communication during the stay. | Supabase (messages). Deleted 90 days after checkout (implemented — see §6). |
...
| guests | first_name (last_name, email columns exist but are unpopulated by current flows), created_at. | Guests | GAP 4a: indefinite. No retention rule. Decision required — e.g. delete or anonymise N days after the last linked booking’s checkout. |
| bookings | check_in, check_out, status, reference token, source, iCal UID; links a guest to an apartment (stay dates of an identifiable person = personal data). | Guests | GAP 4b: indefinite. Hosts have a legitimate business-records interest in booking history — the retention decision (keep dates but sever/anonymise the guest link after N months?) is for the founder + counsel. |
| messages | Free-text message bodies (guest and host), timestamps, read state. | Guests, hosts | IMPLEMENTED: hard-deleted 90 days after the linked booking’s checkout (daily cron, set-based delete, verified live). The only age-based purge of guest data currently in force. |
| daily_greetings | Booking-linked stay-day cache with AI suggestions and weather. | Guests (indirectly, via booking link) | GAP 4c: cascade-deleted only if the booking is deleted; no age purge. Low sensitivity, but should follow whatever bookings decision is made. |
| push_subscriptions | Push endpoint URL + crypto keys; host- or booking-scoped. An endpoint URL identifies a device at a push service. | Hosts, guests | GAP 4d: no purge after checkout for guest subscriptions (5 test rows live). Stale endpoints also accumulate technically. Decide: delete guest rows at/after checkout (aligns with the pre-arrival messaging design, which depends on push). |
...
| admin_audit | Acting admin email, target host ID, action detail JSON. | Hosts, founder | GAP 4e: indefinite. An audit trail arguably should be long-lived; pick and document a period. |
...
- Data minimisation by design: click beacon stores no PII; commission ingest is field-allowlisted; message retention cron (90 days post-checkout); demo accounts fully purged 1 day after expiry including their auth users and guests.
...
4. Retention decisions needed for: (a) guests rows, (b) bookings guest-link, (c) daily_greetings age purge, (d) guest push_subscriptions post-checkout, (e) admin_audit period. These decisions are the prerequisite for building the Art. 17 erasure feature correctly.
