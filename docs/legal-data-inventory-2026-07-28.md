...
| B10. Weather on the guest page | No guest data is involved. Bemgu’s own server sends the apartment’s coordinates (rounded to ~1km) to the weather service and returns four display fields; the guest’s browser never contacts it, so the guest’s IP address is not disclosed. Proxied server-side 8 September 2026 (D4). | Show current weather. | wttr.in (contacted by Bemgu’s server, with property coordinates only — no guest identifier, IP or user-agent). |
...
| admin_audit | Acting admin email, target host ID, action detail JSON. | Hosts, founder | IMPLEMENTED 11 Aug 2026: hard-deleted 365 days after row creation (`cleanup_admin_audit`, daily cron). Bemgu is CONTROLLER here and the table holds no guest data, which is why the period is a year rather than a month. **365 days confirmed for v1 (D2, 1 Sep 2026); counsel to confirm in the fast-follow review.** Formerly GAP 4e: indefinite. |
...
| ntfy.sh | Founder ops alerts (billing events, cron failures, and abuse/spend detector alerts — these MAY include a host account UUID, pseudonymous) and pseudonymous operational heartbeats on guest page opens (added 24 Aug 2026) | Heartbeat payloads carry property-level facts ONLY: property name (host-authored free text), area (neighbourhood/city/country), the resolution state and the entry path. No guest identifiers, no access token or confirmation code, no stay dates, no IP, no host identifier. The event is nonetheless guest-DERIVED — an open on a given property at a given time is one join from the booking, so it is pseudonymous personal data rather than anonymous. GAP 3: payload contents not yet audited for personal data. ntfy topics are effectively public if the topic name is guessable — if any alert includes a host email or name, this is a disclosure. Verify payloads are IDs/counts only, or move alerts to email. | ntfy.sh public instance (location: verify). | Resolve GAP 3 first |
...
- ~~wttr.in~~ — **NO LONGER A CLIENT-SIDE DISCLOSURE. Closed 8 September 2026 (D4):** the weather call was moved behind `api/weather.ts`, so the guest’s browser never contacts the weather service and no guest IP is disclosed. It is recorded at B10 as a Bemgu-side call carrying property coordinates only.
...
- No third-party analytics, advertising, tracking pixels or third-party cookies exist anywhere in the product (verified: no analytics SDK in the codebase), and nothing tracks or profiles an identified guest. The one qualification, added 24 Aug 2026: the operator receives a server-side operational heartbeat when a guest page resolves, carrying property-level data only (property, area, state, entry path) and no guest identifier — see §7.1. The only browser storage used is functional (guest access token, UI preferences).
...
9. ~~wttr.in weather is fetched by the guest’s browser directly~~ — **CLOSED 8 September 2026 (D4): proxied server-side via `api/weather.ts`.** No guest IP reaches the weather service; the processing was stopped rather than disclosed.
